...
# e.g. Results_out$frac[[qq]] (qq: question name) will return for values for Corr/Err/NoI/Skp
	...
	Results_frac <- Results_counts # new data frame for fraction of responses

	Results_out <- list() # two data frames will be output
	...
	for (qq in q_names)
	{
		...
		# all responses
		Results_counts["Cor",qq] <- length( which( data[[qq]] == correct_now ) ) # reponse correct	
		Results_counts["Err",qq] <- length( which( data[[qq]] %in% incorr_now) ) # incorrect responses
		Results_counts["NoI",qq] <- length( which( data[[qq]] == noidea ) ) # response "no idea"
		Results_counts["Skd",qq] <- length( which( is.na( data[[qq]] ) ) ) # response skipped
...
# bar_legend: list of strings, what to use as legend in bar graphs, for items in "groups"
    ...
    bardat <- barplot(data_mats, names=bar_names, beside=T, legend=bar_legend) # bar plot with labels
...
# bar_legend: what to use as legend in bar graphs, for items in "groups"
    ...
    {
        data_mats <- matrix(0,n_groups,4) # number of n_groups x n_bars
        ...
        {
            g_cnt <- g_cnt + 1
            data_mats[g_cnt,] <- matrix(data[[gg]][[rr]][[quest]],1,4)
        }
            
        ### error bars are NONSENSE at the moment, SD doesn't make sense
        # SD <- sd(Dat)
        bardat <- barplot(data_mats, names=names, beside=T, legend=bar_legend) # bar plot with labels
        # arrows(bardat,Dat+SD, bardat, Dat, angle=90, code=1, length=0)
        title( my_title )
    }
}